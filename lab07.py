...
def non_vigenere_index(key_letter, cipher_text, alphabet):
    return index_to_letter(
        (letter_to_index(cipher_text, alphabet) -
         letter_to_index(key_letter, alphabet) +
         len(alphabet)) % len(alphabet), alphabet)
...
def dump_dec_menu(encrypted_list):
    for cipher_text in encrypted_list:
        print(cipher_text)
...
def main():
    ...
    menu = [
        ...
        ['3). Dump Decrypt', dump_dec_menu, [encrypted_list]],
        ['4). Quit', exit, [0]]
    ]
    while True:
        print("-"*80)
        ...
        try:
            ...
            print("Improper choice, please enter an integer between 1 and 4")
    #for _ in range(3):
